...
class ConfigsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create config" do
    assert_difference("Config.count") do
      post configs_url, params: {config: {app_id: @config.app_id, content: @config.content, description: @config.description, env: @config.env, name: @config.name}}
    end
  ...
  test "should update config" do
    patch config_url(@config), params: {config: {app_id: @config.app_id, content: @config.content, description: @config.description, env: @config.env, name: @config.name}}
